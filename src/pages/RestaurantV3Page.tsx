...
export function RestaurantV3Page() {
  ...
  const add = (item: ApiMenuItem) => cart.addItem({ id: item._id, restaurantId: item.restaurant, name: item.name, description: item.description ?? '', price: item.price, category: item.category, image: resolveImageUrl(item.imageUrl) ?? '/images/food/sri_lankan_feast.jpg' })
  return <div className="page container"><Link className="back-link" to="/restaurants"><ArrowLeft /> Back to restaurants</Link><section className="restaurant-hero connected-restaurant"><div className="connected-cover">{restaurant.imageUrl ? <img src={resolveImageUrl(restaurant.imageUrl)} alt={`${restaurant.name} signature food`} /> : <Store />}</div><div><span className="eyebrow">{restaurant.isOpen ? 'Accepting orders' : 'Currently closed'}</span><h1>{restaurant.name}</h1><p>{restaurant.description}</p><div className="restaurant-details"><span><MapPin /> {restaurant.address}</span><span><Phone /> {restaurant.phone}</span><span><Clock3 /> Check menu availability below</span></div></div>{cart.count > 0 && <aside className="cart-preview"><strong>Your cart</strong><span>{cart.count} items</span><b>{formatLkr(cart.subtotal)}</b><Link className="button button--secondary" to="/cart">View cart</Link></aside>}</section><div className="section-heading menu-heading"><div><span className="eyebrow">Fresh from the kitchen</span><h2>Menu</h2></div></div>{menuResource.loading ? <LoadingState label="Loading today’s menu…" /> : menuResource.error ? <ErrorState message={menuResource.error} retry={menuResource.retry} /> : menuResource.data?.length === 0 ? <EmptyState title="Menu unavailable" message="This restaurant has no available menu items right now." /> : <div className="connected-menu-grid">{menuResource.data?.map((item) => <article key={item._id}><div>{item.imageUrl ? <img loading="lazy" src={resolveImageUrl(item.imageUrl)} alt={item.name} /> : <Store />}</div><span>{item.category}</span><h3>{item.name}</h3><p>{item.description}</p><footer><strong>{formatLkr(item.price)}</strong><Button disabled={!restaurant.isOpen} onClick={() => add(item)}>Add to cart</Button></footer></article>)}</div>}</div>
}
