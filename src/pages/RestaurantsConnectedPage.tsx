...
export function RestaurantsConnectedPage() {
  ...
  return <div className="page container">
    ...
    <div className={`restaurant-results-layout ${filtersOpen ? 'filters-open' : ''}`}>
      ...
      <div className="restaurant-results-grid">{resource.loading ? <LoadingState label="Finding restaurants near youâ€¦" /> : resource.error ? <ErrorState message={resource.error} retry={resource.retry} /> : !visible.length ? <EmptyState title="No restaurants found" message="Try another search or filter combination." /> : <div className="api-restaurant-grid">{visible.map((restaurant) => <Link to={`/restaurants/${restaurant._id}`} key={restaurant._id}><div className="api-restaurant-image">{restaurant.imageUrl ? <img loading="lazy" src={resolveImageUrl(restaurant.imageUrl)} alt={`${restaurant.name} logo`} /> : <Store />}<span className={restaurant.isOpen ? 'open' : 'closed'}>{restaurant.isOpen ? 'Open' : 'Closed'}</span></div><div><span>{restaurant.category}</span><h2>{restaurant.name}</h2><p>{restaurant.description}</p><small><Star /> Rating unavailable</small><small><Clock3 /> Delivery time unavailable</small><small><MapPin /> {restaurant.address}</small><small className="restaurant-price-level">{priceText(restaurant.menuPrices)}</small></div></Link>)}</div>}{!resource.loading && pageCount > 1 && <nav className="wireframe-pagination" aria-label="Restaurant pages"><button disabled={page === 1} onClick={() => setPage(page - 1)}>â€¹</button>{Array.from({ length: pageCount }, (_, index) => index + 1).map((number) => <button className={page === number ? 'active' : ''} onClick={() => setPage(number)} key={number}>{number}</button>)}<button disabled={page === pageCount} onClick={() => setPage(page + 1)}>â€º</button></nav>}</div>
    </div>
  </div>
}
