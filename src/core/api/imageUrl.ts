...
export function restaurantImageUrl(value?: string): string | undefined {
  if (!value?.trim()) return undefined
  try {
    const api = new URL(environment.apiBaseUrl, window.location.origin)
    const result = new URL(value, `${api.origin}/`)
    ...
    if (['localhost', '127.0.0.1'].includes(result.hostname)) return `${api.origin}${result.pathname}${result.search}`
    return result.href
