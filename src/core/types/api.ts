...
export type CheckoutInput = { restaurant: string; items: Array<{ menuItem: string; quantity: number }>; deliveryAddress: string; note?: string }
export type CheckoutAttempt = { checkoutId: string; paymentIntentId: string; clientSecret: string; status: string; quote: { subtotal: number; deliveryFee: number; totalAmount: number; deliveryAddress: string; items: ApiOrderItem[] } }
export type ApiOrder = { subtotal?: number; deliveryFee: number; paymentMethod?: string; placedAt?: string; confirmedAt?: string; preparingAt?: string; readyForPickupAt?: string; riderAssignedAt?: string; pickedUpAt?: string; outForDeliveryAt?: string; deliveredAt?: string; cancelledAt?: string;
...
export type RestaurantInput = Pick<ApiRestaurant, 'name' | 'description' | 'address' | 'phone' | 'category' | 'imageUrl' | 'isOpen' | 'operatingHours'>
