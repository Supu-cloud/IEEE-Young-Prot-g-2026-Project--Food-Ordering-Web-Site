...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const applyApiCart = (cart: ApiCart) => setLines(cart.items.map(({ menuItem, quantity }) => ({ id: menuItem._id, restaurantId: menuItem.restaurant, name: menuItem.name, description: menuItem.description ?? '', price: menuItem.price, category: menuItem.category, image: menuItem.imageUrl ?? '/images/food/sri_lankan_feast.jpg', quantity })))
  ...
  const addItem = (item: MenuItem) => setLines((current) => {
    ...
    if (!environment.useMocks && user?.role === 'customer') void cartApi.add(item.id).then(applyApiCart).catch((error)=>setSyncError(error instanceof Error?error.message:'Unable to update cart'))
    return next
  })
  ...
  const clearCart = () => { setLines([]); if (!environment.useMocks && user?.role === 'customer') void cartApi.clear().catch((error)=>setSyncError(error instanceof Error?error.message:'Unable to clear cart')) }
  const value = { lines, count: lines.reduce((sum, line) => sum + line.quantity, 0), subtotal: lines.reduce((sum, line) => sum + line.price * line.quantity, 0), addItem, setQuantity, removeItem, clearCart, syncError }
